...
import { Plus, Archive, Edit3, Calendar, Download} from 'lucide-react';
...
import { Perda, PerdaData } from '../../types';
...
const PerdaManagement: React.FC = () => {
 ...
 const { loading: loadingProdutos } = useProdutos();
 const { expiredProducts } = useNotifications();
 ...
 const isLoading = loadingPerdas || loadingProdutos;

 const allPerdas = useMemo(() => {
  const vencidasComoPerdas: Perda[] = expiredProducts.map((p, index) => ({
   id: -1 * (index + 1),
   produto: p,
   produtoId: p.id,
