...
import { X, Package, UploadCloud, Trash2, Tag as TagIcon } from 'lucide-react';
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-5 border-b dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-800 z-10">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">{isEditing ? 'Editar Promoção' : 'Criar Promoção'}</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 focus:outline-none" disabled={loading}><X className="w-6 h-6" /></button>
        </div>

        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
          <div>
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Informações da Promoção</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="md:col-span-2">
                <label htmlFor="promo-descricao" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Descrição *</label>
              ...
              <div>
                <label htmlFor="promo-desconto" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Desconto (%) *</label>
              ...
              <div className="flex items-end pb-2">
                <input id="promo-ativa" type="checkbox" checked={formData.ativa} onChange={(e) => handleInputChange('ativa', e.target.checked)} className="h-4 w-4 text-purple-600 border-gray-300 dark:border-gray-600 rounded focus:ring-offset-0 focus:ring-2 focus:ring-purple-500" disabled={loading} />
                <label htmlFor="promo-ativa" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100 select-none">Promoção ativa</label>
              </div>
            </div>
          </div>

          <div>
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Período da Promoção</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <label htmlFor="promo-dataInicio" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Data de Início *</label>
              ...
              <div>
                <label htmlFor="promo-dataFim" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Data de Fim *</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              <UploadCloud className="w-4 h-4 inline mr-1 text-gray-500 dark:text-gray-400" /> Imagem da Promoção (Opcional)
            </label>
            <div className="mt-1 flex flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
              <div className="flex-shrink-0 h-20 w-20 rounded-md overflow-hidden bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-gray-400 dark:text-gray-500 border dark:border-gray-600">
                {imagePreview ? (
              ...
              <div className="flex-1 w-full">
                <input
                  ...
                  className="block w-full text-sm text-gray-500 dark:text-gray-300 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100 dark:file:bg-purple-900/50 dark:file:text-purple-300 dark:hover:file:bg-purple-900 cursor-pointer border border-gray-300 dark:border-gray-600 rounded-md"
                  disabled={loading}
                />
          ...
          <div>
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4"><Package className="w-5 h-5 inline mr-2" /> Produtos da Promoção</h3>
            <p className="text-sm text-gray-600 dark:text-gray-300 mb-3">Selecione os produtos que farão parte desta promoção.</p>
            <div className="mb-4">
            ...
            <div className="border border-gray-300 dark:border-gray-600 rounded-md max-h-60 overflow-y-auto">
              {loadingProdutos ? (
                <div className="p-4 text-center text-gray-500 dark:text-gray-400">Carregando produtos...</div>
              ) : filteredProdutos.length === 0 ? (
                <div className="p-4 text-center text-gray-500 dark:text-gray-400">Nenhum produto encontrado</div>
              ) : (
                <div className="divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredProdutos.map((produto) => (
                    <div key={produto.id} className="p-3 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      <label className="flex items-center cursor-pointer">
                        <input type="checkbox" checked={produtosSelecionados.includes(produto.id)} onChange={() => handleProductToggle(produto.id)} className="w-4 h-4 text-purple-600 border-gray-300 dark:border-gray-600 rounded focus:ring-purple-500" disabled={loading} />
                        <div className="ml-3 flex-1">
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{produto.nome}</span>
                            <span className="text-sm text-gray-500 dark:text-gray-400">{produto.precoVenda ? formatCurrency(produto.precoVenda) : ''}</span>
                          </div>
                          <p className="text-xs text-gray-500 dark:text-gray-400">{produto.categoria?.nome}</p>
                        </div>
                      </label>
                    </div>
                  ))}
            ...
            <div className="mt-2 text-sm text-gray-600 dark:text-gray-300">{produtosSelecionados.length} produto(s) selecionado(s)</div>
          </div>

          <div className="flex justify-end space-x-4 pt-6 border-t dark:border-gray-700 mt-6 sticky bottom-0 bg-white dark:bg-gray-800 py-4 px-6 z-10">
            <button type="button" onClick={onClose} className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700" disabled={loading}>Cancelar</button>
            <button type="submit" disabled={loading} className="inline-flex justify-center items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-purple-600 hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed">
              {loading && (
      ...
      <style>{`
    .input { @apply w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm disabled:bg-gray-100 disabled:cursor-not-allowed dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-gray-100 dark:disabled:bg-gray-700/50; }
    .input-error { @apply border-red-500 focus:ring-red-500; }
    .select { @apply w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent bg-white text-sm disabled:bg-gray-100 disabled:cursor-not-allowed dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-gray-100 dark:disabled:bg-gray-700/50; }
    .textarea { @apply w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm disabled:bg-gray-100 disabled:cursor-not-allowed dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-gray-100 dark:disabled:bg-gray-700/50; }
   `}</style>
    </div>
  );
};
