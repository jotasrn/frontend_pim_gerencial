...
interface PromotionFormProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (promotionData: any) => void;
  initialData?: any;
  isEditing?: boolean;
}
...
const PromotionForm: React.FC<PromotionFormProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    percentualDesconto: initialData?.percentualDesconto || '',
    dataInicio: initialData?.dataInicio || '',
    dataFim: initialData?.dataFim || '',
    ativa: initialData?.ativa ?? true,
    produtosSelecionados: initialData?.produtos?.map((p: any) => p.id) || [],
  });
  ...
  const validateForm = () => {
    ...
    if (formData.percentualDesconto) {
      ...
      if (desconto <= 0 || desconto > 100) {
        newErrors.percentualDesconto = 'Desconto deve estar entre 1% e 100%';
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData = {
        ...
        produtoIds: formData.produtosSelecionados,
      };
      
      await onSubmit(submitData);
      showToast.success(isEditing ? 'Promoção atualizada com sucesso!' : 'Promoção criada com sucesso!');
      onClose();
    } catch (error) {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-xl font-semibold text-gray-900">
            {isEditing ? 'Editar Promoção' : 'Criar Promoção'}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  placeholder="Ex: Promoção de Frutas da Semana"
                />
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 ${
                    errors.percentualDesconto ? 'border-red-500' : 'border-gray-300'
                  }`}
                  placeholder="0.00"
                />
          ...
          <div>
            ...
            <div className="mt-2 text-sm text-gray-600">
              {formData.produtosSelecionados.length} produto(s) selecionado(s)
            </div>
          </div>
